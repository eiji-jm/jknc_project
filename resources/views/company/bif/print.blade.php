    ...
    <div class="print-shell mx-auto max-w-6xl p-4 md:p-6">
        <div class="no-print mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3">
            ...
            <div class="flex gap-2">
                <button type="button" onclick="window.location.href='{{ $backUrl ?? route('company.bif.show', ['company' => $company->id, 'bif' => $bif->id]) }}'"
        ...
        <div class="document-page">
            @include('company.bif.partials.document', ['wrapperClass' => 'bif-doc bif-print-document mx-auto w-full max-w-5xl'])
        </div>
    </div>

    <script>
        const params = new URLSearchParams(window.location.search);
        if (params.get('autoprint') === '1' || @json(!empty($autoPrint))) {
