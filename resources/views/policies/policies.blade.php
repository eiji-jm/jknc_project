    ...
    <div class="w-full px-6 py-4">
        ...
        <div class="overflow-x-auto border border-gray-300 bg-white">
            <table class="min-w-[1400px] w-full border-collapse text-sm text-gray-800">
                ...
                <tbody>
                    {{-- Sample empty rows to mimic spreadsheet UI --}}
                    @forelse(($policies ?? []) as $policy)
                        <tr class="hover:bg-gray-50">
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->status ?? '-' }}</td>
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->code ?? '-' }}</td>
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->version ?? '-' }}</td>
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->effectivity_date ?? '-' }}</td>
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->prepared_by ?? '-' }}</td>
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->reviewed_by ?? '-' }}</td>
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->approved_by ?? '-' }}</td>
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->review_cycle ?? '-' }}</td>
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->review_date ?? '-' }}</td>
                            <td class="border border-gray-300 px-4 py-3">{{ $policy->classification ?? '-' }}</td>
...
<div id="addPolicyModal" class="hidden fixed inset-0 z-50">
    ...
    <div class="relative z-10 flex items-center justify-center min-h-screen p-4">
        <div class="w-full max-w-4xl bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden">
            ...
            <form action="{{ route('policies.store') }}" method="POST" class="p-6">
                @csrf
