                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse(($policies ?? []) as $policy)
                            <tr class="hover:bg-blue-50/30 transition-colors">
                                <td class="px-4 py-3 border-r border-gray-100">{{ $policy->status ?? 'Draft' }}</td>
    ...
    <div x-show="showSlideOver" x-cloak class="fixed inset-0 z-[60] overflow-hidden">
        ...
        <div class="absolute inset-0 flex">
            ...
            >
                ...
                <form id="policyFormSubmit" method="POST" action="{{ route('policies.store') }}" class="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                    @csrf
                    ...
                    {{-- RICH TEXT DESCRIPTION (Townhall Body Style) --}}
                    <div>
...
<script>
    ...
    function updatePreview() {
        ...
        const pdfUrl = `{{ route('policies.preview') }}?${params.toString()}`;
