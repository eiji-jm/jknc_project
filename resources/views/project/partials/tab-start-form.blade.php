@php
    ...
    $startFormId = $startFormId ?? 'project-start-form';
@endphp

<form id="{{ $startFormId }}" method="POST" action="{{ $startFormAction ?? route('project.start.update', $project) }}" class="space-y-4">
    @csrf
    ...
    <section class="project-doc-shell overflow-hidden bg-white p-2 text-slate-900">
        <div class="mx-auto max-w-[980px] border-[3px] border-[#163b7a] bg-white">
            ...
            <div class="border-t border-slate-900">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-3 py-3">
                    <div class="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">Requirement Rows</div>
                    <button type="button" class="project-doc-action" data-add-row="start-requirements">
                        <i class="fas fa-plus text-xs"></i>Add Engagement Row
                    </button>
                </div>
                    ...
                    <tbody id="start-requirements">
                        @foreach ($startReqs as $index => $item)
                            <tr class="start-matrix-row">
                                <td class="border border-slate-900 px-2 py-1 text-center start-index">{{ $index + 1 }}</td>
            ...
            <div class="grid gap-4 border-t border-slate-900 bg-slate-50/70 p-3">
                <div>
                    <label class="block text-[10px] font-bold uppercase tracking-wide text-slate-600">Rejection / Hold Reason</label>
                    <input name="rejection_reason" value="{{ $rejectionReason }}" class="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm" {{ $startStatus === 'pending_approval' ? 'readonly' : '' }}>
                </div>
                <div class="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-3">
                    <div class="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">Form Actions</div>
                    <div class="flex flex-wrap items-center justify-end gap-3">
                        @if ($project?->id && in_array($startStatus, ['draft', 'rejected'], true))
                            <button type="submit" formaction="{{ route('project.start.submit', $project) }}" class="project-doc-action">
                                <i class="fas fa-paper-plane text-xs"></i>Submit for Approval
                            </button>
                        @endif
                        <button type="submit" class="project-doc-primary">
                            <i class="fas fa-floppy-disk text-xs"></i>Save START Form
                        </button>
                    </div>
                </div>
            </div>
