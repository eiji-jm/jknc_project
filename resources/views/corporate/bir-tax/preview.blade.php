...
@php
    ...
    $draftUrl = $tax->document_path ? $uploadUrl($tax->document_path) : ($generatedDraftUrl ?? null);
    $approvedUrl = $tax->approved_document_path ? $uploadUrl($tax->approved_document_path) : null;
    $draftDocuments = collect($tax->draft_documents ?? [])->filter(fn ($entry) => !empty($entry['path']))->values();
    $approvedDocuments = collect($tax->approved_documents ?? [])->filter(fn ($entry) => !empty($entry['path']))->values();
