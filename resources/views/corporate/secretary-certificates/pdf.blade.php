...
<body>
    ...
    <div class="content">
        ...
        <div class="resolution-title">{{ $certificate->resolution_no ? 'BOARD RESOLUTION NO. ' . $certificate->resolution_no : 'CERTIFIED MINUTES EXTRACT' }}</div>
        <p><strong>{{ $certificate->purpose ?: 'Corporate Purpose' }}</strong></p>
        <p>{!! $certificate->resolution_body ?: nl2br(e($resolution?->resolution_body ?: ('Certified from Minutes Ref. ' . ($certificate->minutes_ref ?: ($minute?->minutes_ref ?: '________________')) . '.'))) !!}</p>
