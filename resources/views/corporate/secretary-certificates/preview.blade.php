...
<style>
    @media print {
        ...
        #sec-print, #sec-print * { visibility: visible; }
        #sec-print { position: absolute; left: 0; top: 0; width: 100%; }
    }
</style>
    ...
    <div class="bg-white border border-gray-100 rounded-xl overflow-hidden">
        <div class="flex items-center gap-3 px-4 py-4 border-b border-gray-100">
            ...
            <div>
                ...
                <div class="text-xs text-gray-500">Certificate No. {{ $certificate->certificate_no ?? '—' }}</div>
            </div>
            <div class="flex-1"></div>
            <a href="{{ $editRoute }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg">Edit</a>
            <form method="POST" action="{{ $deleteRoute }}" onsubmit="return confirm('Delete this certificate?');">
                @csrf
                @method('DELETE')
            ...
            <div class="lg:col-span-3 space-y-4">
                <div class="bg-gray-900 rounded-xl overflow-hidden">
                    <div class="bg-gray-800 px-4 py-3 flex items-center gap-2 border-b border-gray-700">
                        <span class="text-gray-300 text-sm font-medium">Secretary Certificate</span>
                        <div class="flex-1"></div>
                    ...
                    <div class="p-6 overflow-auto">
                        <div id="sec-print" class="bg-white w-full max-w-md rounded-sm shadow-2xl mx-auto" style="aspect-ratio: 8.5/11;">
                            <div class="p-8 h-full flex flex-col justify-between text-center">
                                <div class="border-b-2 border-gray-800 pb-4 mb-4">
                                    <h1 class="text-xl font-bold text-gray-900">SECRETARY CERTIFICATE</h1>
                                    <p class="text-xs text-gray-600 mt-2">{{ $certificate->governing_body ?? '—' }}</p>
                                </div>
                                <div class="flex-1 flex flex-col justify-center space-y-3 text-xs text-gray-700">
                                    ...
                                    <p><strong>Meeting:</strong> {{ $certificate->type_of_meeting ?? '—' }} | {{ optional($certificate->date_of_meeting)->format('M d, Y') }}</p>
                                    <p><strong>Location:</strong> {{ $certificate->location ?? '—' }}</p>
                                </div>
                                <div class="border-t-2 border-gray-800 pt-3 text-xs text-gray-600">
                                    Secretary: {{ $certificate->secretary ?? '—' }}
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        <div><span class="text-xs text-gray-600 uppercase tracking-wide">Certificate No.</span><div class="font-medium text-gray-900">{{ $certificate->certificate_no ?? '—' }}</div></div>
                        ...
                        <div><span class="text-xs text-gray-600 uppercase tracking-wide">Purpose</span><div class="font-medium text-gray-900">{{ $certificate->purpose ?? '—' }}</div></div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 border border-gray-200 rounded-xl p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        <div><span class="text-xs text-gray-600 uppercase tracking-wide">Resolution No.</span><div class="font-medium text-gray-900">{{ $certificate->resolution_no ?? '—' }}</div></div>
                        ...
                        <div><span class="text-xs text-gray-600 uppercase tracking-wide">Meeting No.</span><div class="font-medium text-gray-900">{{ $certificate->meeting_no ?? '—' }}</div></div>
                    </div>
                </div>
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        <div><span class="text-xs text-gray-600 uppercase tracking-wide">Notary Public</span><div class="font-medium text-gray-900">{{ $certificate->notary_public ?? '—' }}</div></div>
                        <div><span class="text-xs text-gray-600 uppercase tracking-wide">Doc / Page / Book / Series</span><div class="font-medium text-gray-900">{{ $certificate->notary_doc_no ?? '—' }} / {{ $certificate->notary_page_no ?? '—' }} / {{ $certificate->notary_book_no ?? '—' }} / {{ $certificate->notary_series_no ?? '—' }}</div></div>
                    </div>
                </div>
