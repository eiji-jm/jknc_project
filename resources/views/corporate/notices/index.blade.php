...
<div class="w-full px-4 sm:px-6 lg:px-8 mt-4" x-data="{ showAddPanel: false }" @keydown.escape.window="showAddPanel = false">
    ...
    @forelse ($notices as $notice)
        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer" onclick="window.location='{{ route('notices.preview', $notice) }}'">
            <td class="px-4 py-3 font-medium">{{ $notice->notice_number }}</td>
            <td class="px-4 py-3">{{ optional($notice->date_of_notice)->format('M d, Y') }}</td>
            <td class="px-4 py-3">{{ $notice->governing_body }}</td>
            <td class="px-4 py-3">{{ $notice->type_of_meeting }}</td>
            <td class="px-4 py-3">{{ optional($notice->date_of_meeting)->format('M d, Y') }}</td>
            <td class="px-4 py-3">{{ $notice->time_started }}</td>
            <td class="px-4 py-3">{{ $notice->location }}</td>
            <td class="px-4 py-3">{{ $notice->meeting_no }}</td>
            <td class="px-4 py-3">{{ $notice->chairman }}</td>
            <td class="px-4 py-3">{{ $notice->secretary }}</td>
            <td class="px-4 py-3">{{ $notice->uploaded_by }}</td>
            <td class="px-4 py-3">{{ optional($notice->date_updated)->format('M d, Y') }}</td>
        </tr>
    @empty
        <tr>
            <td colspan="12" class="px-4 py-6 text-center text-sm text-gray-500">No notices found.</td>
        </tr>
    @endforelse
</tbody>
                </table>
            ...
            <div class="p-6 overflow-y-auto space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm" placeholder="Board of Directors">
                    </div>
                    <div>
