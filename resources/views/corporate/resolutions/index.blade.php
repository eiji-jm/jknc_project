...
<div class="w-full px-4 sm:px-6 lg:px-8 mt-4" x-data="{ showAddPanel: false }" @keydown.escape.window="showAddPanel = false">
    ...
    @forelse ($resolutions as $resolution)
        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer" onclick="window.location='{{ route('resolutions.preview', $resolution) }}'">
            ...
            <td class="px-4 py-3">{{ optional($resolution->date_uploaded)->format('M d, Y') }}</td>
            <td class="px-4 py-3">{{ $resolution->uploaded_by }}</td>
            <td class="px-4 py-3">{{ $resolution->governing_body }}</td>
            <td class="px-4 py-3">{{ $resolution->type_of_meeting }}</td>
            <td class="px-4 py-3">{{ $resolution->notice_ref }}</td>
            <td class="px-4 py-3">{{ $resolution->meeting_no }}</td>
            <td class="px-4 py-3">{{ optional($resolution->date_of_meeting)->format('M d, Y') }}</td>
            <td class="px-4 py-3">{{ $resolution->location }}</td>
            <td class="px-4 py-3">{{ $resolution->board_resolution }}</td>
            ...
            <td class="px-4 py-3">{{ $resolution->notary_public }}</td>
        </tr>
    @empty
        <tr>
            <td colspan="14" class="px-4 py-6 text-center text-sm text-gray-500">No resolutions found.</td>
        </tr>
    @endforelse
</tbody>
                </table>
