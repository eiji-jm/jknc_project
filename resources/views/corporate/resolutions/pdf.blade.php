...
<head>
    ...
    <title>Resolution {{ $selected->resolution_no ?: 'Draft Resolution' }}</title>
    <style>
        @page {
            size: A4;
...
<body>
    <div class="page">
        ...
        <div class="title">Board Resolution No. {{ $resolutionNumber }}</div>

        <div class="content">
            <p><strong>WHEREAS,</strong> during the {{ $meetingType }} meeting of the {{ $governingBody }} of <strong><u>{{ $companyName }}</u></strong> held on <strong><u>{{ $meetingDate }}</u></strong>, where a quorum was present and acted all throughout, the body approved the following action:</p>

            <p>{!! nl2br(e($clauseText ?: ($selected->board_resolution ?: 'No board resolution text has been encoded yet.'))) !!}</p>
            ...
            <p><strong><u>FINALLY BE IT FURTHER RESOLVED</u></strong> that the undersigned affirm the foregoing resolution and adopt it on this <strong><u>{{ $meetingDate }}</u></strong>.</p>
        </div>

        <div class="signature-block">
            <div class="signature-line">
                <div style="font-weight:700; text-transform:uppercase;">{{ $chairman }}</div>
                <div>Chairman</div>
            </div>
        </div>

        <div class="content" style="margin-top: 44px;">
            <p>IN WITNESS WHEREOF, I, <strong><u>{{ $chairman }}</u></strong>, in my capacity as chairman of the board, have signed these presents this ______ day of __________ at {{ $notarizedAt }}.</p>
        </div>
        ...
        <div class="notary">
            <p><strong>SUBSCRIBED AND SWORN</strong> to before me, a Notary Public for and in {{ $notarizedAt }} this day of __________, affiant presented to me __________ issued at __________.</p>

            <div class="signature-block" style="margin-top: 30px;">
                <div class="signature-line">
                    <div style="font-weight:700; text-transform:uppercase;">{{ $notaryPublic }}</div>
                </div>
            </div>

            <div class="notary-meta">
                <div>Doc. No. {{ $selected->notary_doc_no ?: '_____' }};</div>
                <div>Page No. {{ $selected->notary_page_no ?: '_____' }};</div>
                <div>Book No. {{ $selected->notary_book_no ?: '_____' }};</div>
                <div>Series of {{ $selected->notary_series_no ?: $notaryYear }}.</div>
            </div>
        </div>
    </div>
</body>
