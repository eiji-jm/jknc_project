...
<div class="w-full px-4 sm:px-6 lg:px-8 mt-4"
                    ...
                    <tbody class="text-sm text-gray-900" id="index-table-body">
                        @forelse($indexes ?? [] as $index)
                            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3">{{ $index->family_name }}</td>
                                <td class="px-4 py-3">{{ $index->first_name }}</td>
                                <td class="px-4 py-3">{{ $index->middle_name }}</td>
                                <td class="px-4 py-3">{{ $index->nationality }}</td>
                                <td class="px-4 py-3">{{ $index->current_address }}</td>
                                <td class="px-4 py-3">{{ $index->tin }}</td>
                            </tr>
                        @empty
                            <tr>
    ...
    <div x-cloak>
        ...
        <div x-show="showAddPanel"
            ...
            <form action="{{ route('stock-transfer-book.index.store') }}"
                  method="POST"
...
<script>
    const tempContacts = @json($tempContacts);
