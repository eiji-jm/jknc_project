...
<div class="rsat-workspace p-6">
    <div class="mx-auto max-w-[1380px] space-y-4">
        ...
        <form method="POST" action="{{ route('regular.rsat.update', $regular) }}" class="rsat-sheet overflow-hidden p-6 {{ $tab !== 'rsat' ? 'hidden' : '' }}" data-tab-panel="rsat">
            ...
            <div class="rsat-form">
                ...
                <div class="mt-5 rsat-client-row">
                    ...
                    <div class="rsat-check-group"><span class="rsat-check">{{ $isNewClient ? 'X' : '' }}</span> NEW CLIENT</div>
                    <div class="rsat-check-group"><span class="rsat-check">{{ $isExistingClient ? 'X' : '' }}</span> EXISTING CLIENT</div>
                    <div class="rsat-check-group"><span class="rsat-check">{{ $isChangeInformation ? 'X' : '' }}</span> CHANGE INFORMATION</div>
                </div>
                    ...
                    <div id="regular-requirements">
                        @foreach ($rsatRequirements as $index => $item)
                            <div class="rsat-matrix-row">
                                <div class="rsat-index">{{ $index + 1 }}</div>
                                <div>
        ...
        <form method="POST" action="{{ route('regular.report.update', $regular) }}" class="rsat-sheet overflow-hidden p-6 {{ $tab !== 'report' ? 'hidden' : '' }}" data-tab-panel="report">
            @csrf
            ...
            <div class="rsat-form">
                ...
                <div class="rsat-matrix">
                    ...
                    <div id="regular-report-rows">
                        @foreach ($reportRows as $index => $item)
                            <div class="rsat-matrix-row">
                                <div class="rsat-index">{{ $index + 1 }}</div>
                                <div><input name="report_service[]" value="{{ old('report_service.'.$index, $item['service'] ?? '') }}" class="rsat-row-input"></div>
                                <div><input name="report_activity_output[]" value="{{ old('report_activity_output.'.$index, $item['activity_output'] ?? '') }}" class="rsat-row-input"></div>
                                <div><input name="report_frequency[]" value="{{ old('report_frequency.'.$index, $item['frequency'] ?? '') }}" class="rsat-row-input"></div>
                                <div><input name="report_reminder_lead_time[]" value="{{ old('report_reminder_lead_time.'.$index, $item['reminder_lead_time'] ?? '') }}" class="rsat-row-input"></div>
                                <div><input name="report_deadline[]" value="{{ old('report_deadline.'.$index, $item['deadline'] ?? '') }}" class="rsat-row-input"></div>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="rsat-signature">
                    <input name="client_confirmation_name" value="{{ $reportClientSignature }}" class="rsat-line-input w-full max-w-[420px] text-center">
                    <div class="rsat-signature-label">Client Fullname &amp; Signature</div>
                </div>
                ...
                <div class="rsat-approval-grid">
                    <div class="rsat-approval-pair">
                        <div class="rsat-approval-label">Prepared By:</div>
                        <input name="prepared_by" value="{{ $reportPreparedBy }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-approval-pair">
                        <div class="rsat-approval-label">Reviewed By:</div>
                        <input name="reviewed_by" value="{{ $reportReviewedBy }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-approval-pair">
                        <div class="rsat-approval-label">Name:</div>
                        <input name="prepared_by_name" value="{{ $reportPreparedByName }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-approval-pair">
                        <div class="rsat-approval-label">Name:</div>
                        <input name="reviewed_by_name" value="{{ $reportReviewedByName }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-approval-pair">
                        <div class="rsat-approval-label">Date:</div>
                        <input name="prepared_by_date" value="{{ $reportPreparedByDate }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-approval-pair">
                        <div class="rsat-approval-label">Date:</div>
                        <input name="reviewed_by_date" value="{{ $reportReviewedByDate }}" class="rsat-line-input">
                    </div>
                </div>
                ...
                <div class="rsat-footer-grid">
                    <div class="rsat-footer-pair">
                        <div>Referred By/Closed By:</div>
                        <input name="referred_by_closed_by" value="{{ $reportReferredBy }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-footer-pair">
                        <div>Sales &amp; Marketing:</div>
                        <input name="sales_marketing" value="{{ $reportSalesMarketing }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-footer-pair">
                        <div>Lead Consultant:</div>
                        <input name="lead_consultant" value="{{ $reportLeadConsultant }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-footer-pair">
                        <div>Lead Associate Assigned:</div>
                        <input name="lead_associate_assigned" value="{{ $reportLeadAssociate }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-footer-pair">
                        <div>Finance:</div>
                        <input name="finance" value="{{ $reportFinance }}" class="rsat-line-input">
                    </div>
                    <div class="rsat-footer-pair">
                        <div>President:</div>
                        <input name="president" value="{{ $reportPresident }}" class="rsat-line-input">
                    </div>
                </div>
                ...
                <div class="mt-4 rsat-footer-grid">
                    <div class="rsat-footer-note">
                        <div class="rsat-footer-pair" style="grid-template-columns: 230px minmax(0, 1fr);">
                            <div>Record Custodian ( Name and Signature)</div>
                            <input name="record_custodian" value="{{ $reportRecordCustodian }}" class="rsat-line-input">
                        </div>
                    </div>
                    <div class="space-y-3">
                        <div class="rsat-footer-pair">
                            <div>Date Recorded:</div>
                            <input type="date" name="date_recorded" value="{{ $reportDateRecorded }}" class="rsat-line-input">
                        </div>
                        <div class="rsat-footer-pair">
                            <div>Date Signed:</div>
                            <input type="date" name="date_signed" value="{{ $reportDateSigned }}" class="rsat-line-input">
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 border-t border-slate-200 pt-6">
                    ...
                    <div class="mt-4 rsat-meta-grid">
                        <div class="rsat-meta-item">
                            ...
                            <input name="transmittal_no" value="{{ $reportTransmittalNo }}" class="rsat-line-input">
                        </div>
                        <div class="rsat-meta-item" style="grid-template-columns: 250px minmax(0, 1fr);">
                            ...
                            <input type="date" name="date_submitted_for_transmittal" value="{{ $reportDateSubmittedForTransmittal }}" class="rsat-line-input">
                        </div>
                    </div>
                </div>
            </div>
