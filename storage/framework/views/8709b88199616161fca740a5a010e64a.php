...
<div class="bg-white p-6">
    ...
    <form method="GET" action="<?php echo e(route('deals.index')); ?>" class="mb-4 flex flex-wrap items-center gap-2 text-sm">
        <div class="relative w-full max-w-sm">
            <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-xs text-gray-400"></i>
            <input type="text" name="search" value="<?php echo e($search ?? ''); ?>" placeholder="Search code, deal, or contact" class="h-9 w-full rounded-lg border border-gray-200 bg-white pl-8 pr-3 text-sm text-gray-700 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
        </div>
        ...
        <span id="totalDealsCount" class="text-xs text-gray-500"><?php echo e($totalDeals); ?> deals</span>
    </form>
    ...
    <div class="overflow-x-auto pb-2">
        <div id="dealsBoard" class="flex min-w-max gap-3">
            <?php $__currentLoopData = $stageColumns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $column): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                    $stageBorderColor = $column['color'] ?: ($column['stage'] === 'Closed Lost' ? '#dc2626' : '#1e293b');
                    $headerClass = 'bg-slate-800';
                ?>
                <section class="stage-column w-[230px] overflow-hidden rounded-xl border border-gray-200 bg-gray-50" data-stage-id="<?php echo e($column['id'] ?? ''); ?>" data-stage-column="<?php echo e($column['stage']); ?>" data-stage-color="<?php echo e($stageBorderColor); ?>">
                    <header class="stage-header group/column rounded-t-xl border-t-4 px-3 py-2 text-white <?php echo e($headerClass); ?>" style="border-top-color: <?php echo e($stageBorderColor); ?>;">
                        <div class="flex items-center justify-between gap-2">
                            <div class="min-w-0">
                                <div class="min-w-0">
                                    <h2 class="stage-name cursor-text text-xs font-semibold"><?php echo e($column['stage']); ?></h2>
                                    <input type="text" class="stage-name-input hidden h-5 w-full rounded border border-white/30 bg-white/10 px-1 text-xs font-semibold text-white outline-none focus:border-white/60" value="<?php echo e($column['stage']); ?>">
                                    <p class="stage-total text-xs opacity-90"><?php echo e($formatCurrency($column['total_amount'])); ?> • <?php echo e(count($column['deals'])); ?> <?php echo e(count($column['deals']) === 1 ? 'Deal' : 'Deals'); ?></p>
                                </div>
                            </div>
                            <div class="relative flex items-start gap-2">
                                ...
                                <button type="button" class="stage-actions-trigger opacity-0 transition group-hover/column:opacity-100" data-stage="<?php echo e($column['stage']); ?>" title="Stage actions">
                                    <i class="fas fa-ellipsis-vertical text-xs text-white/90"></i>
                                </button>
                                <div class="stage-actions-menu absolute right-0 z-20 mt-2 hidden w-48 rounded-lg border border-gray-200 bg-white py-1 text-xs text-gray-700 shadow-lg" data-stage-menu="<?php echo e($column['stage']); ?>">
                                    ...
                                    <div class="border-t border-gray-100 px-3 py-2">
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            <?php $__currentLoopData = ['#2563eb', '#4f46e5', '#0891b2', '#d97706', '#ea580c', '#059669', '#7c3aed', '#dc2626']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $colorOption): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <button type="button" class="stage-color-option h-4 w-4 rounded-full border border-gray-200" data-action="change-color" data-color="<?php echo e($colorOption); ?>" style="background-color: <?php echo e($colorOption); ?>;"></button>
                    ...
                    <div class="stage-cards space-y-2 p-2">
                        <?php $__empty_1 = true; $__currentLoopData = $column['deals']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $deal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <?php
                            ...
                            <article
                                ...
                                data-deal-name="<?php echo e($deal['deal_code'] ?? $deal['deal_name']); ?>"
                                data-deal-stage="<?php echo e($column['stage']); ?>"
                                data-deal-contact="<?php echo e($deal['contact_name']); ?>"
                                data-deal-company="<?php echo e($deal['company_name']); ?>"
                                data-deal-amount="<?php echo e($deal['amount']); ?>"
                                data-deal-expected-close="<?php echo e($deal['expected_close']); ?>"
                                data-deal-code="<?php echo e($deal['deal_code'] ?? 'Auto-generated after save'); ?>"
                                data-deal-created-by="<?php echo e($deal['created_by'] ?? 'System'); ?>"
                                data-deal-created-at="<?php echo e($deal['created_at_label'] ?? now()->format('F d, Y • h:i:s A')); ?>"
                                data-view-url="<?php echo e(route('deals.show', $deal['id'])); ?>"
                                data-edit-url="<?php echo e(route('deals.update', $deal['id'])); ?>"
                                >
                                <label class="deal-card-checkbox pointer-events-none absolute right-9 top-2 z-10 flex h-5 w-5 items-center justify-center rounded border border-gray-200 bg-white/95 shadow-sm opacity-0 transition duration-150 group-hover/deal:pointer-events-auto group-hover/deal:opacity-100">
                                    <input type="checkbox" class="deal-select-checkbox h-3.5 w-3.5" data-deal-select="<?php echo e($deal['id']); ?>">
                                </label>
                                ...
                                <a
                                    id="deal-<?php echo e($deal['id']); ?>"
                                    href="<?php echo e(route('deals.show', $deal['id'])); ?>"
                                    class="block"
                                >
                                    <h3 class="pr-16 text-[14px] font-semibold tracking-wide text-blue-700 [overflow-wrap:break-word] line-clamp-2"><?php echo e($deal['deal_code'] ?? 'DEAL'); ?></h3>
                                    <p class="mt-2 text-xs text-gray-700"><?php echo e($deal['contact_name']); ?></p>
                                    <p class="text-[11px] text-gray-400"><?php echo e($deal['company_name']); ?></p>

                                    <p class="mt-3 text-lg font-semibold text-blue-700"><?php echo e($formatCurrency($deal['amount'])); ?></p>
                                    <p class="mt-2 text-[11px] text-gray-400">Expected Close</p>
                                    <p class="text-xs text-gray-700"><?php echo e($deal['expected_close']); ?></p>

                                    <div class="mt-3 flex items-center gap-1.5 border-t border-gray-100 pt-2 text-[11px] text-gray-500">
                                        <span class="flex h-4 w-4 items-center justify-center rounded-full bg-blue-100 text-[9px] font-semibold text-blue-700"><?php echo e($ownerInitials); ?></span>
                                        <span><?php echo e($deal['owner_name']); ?></span>
                                    </div>
                                </a>
                            </article>
                    ...
                    <div class="border-t border-gray-200 px-2 py-2">
                        <p class="stage-description text-[11px] text-gray-500" data-stage-description="<?php echo e($column['stage']); ?>">No description yet.</p>
                    </div>
                </section>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const defaultDealInfo = {
        ...
        createdBy: <?php echo json_encode(auth()->user()->name ?? 'System', 15, 512) ?>,
        createdAt: <?php echo json_encode(now()->format('F d, Y • h:i:s A'), 512) ?>,
    };
    ...
    const setFormModeCreate = () => {
        ...
        form.action = <?php echo json_encode(route('deals.store'), 15, 512) ?>;
    ...
    const finishInlineRename = async (section, shouldSave) => {
        ...
        if (shouldSave && nextName !== '' && nextName !== originalName && section.dataset.stageId) {
            const payload = await api(`<?php echo e(url('/deals/stages')); ?>/${section.dataset.stageId}`, 'PATCH', { name: nextName });
    ...
    const handleCreateStage = async () => {
        ...
        const payload = await api(`<?php echo e(route('deals.stages.store')); ?>`, 'POST', { name: stageName });
    ...
    stageDeleteConfirm?.addEventListener('click', async () => {
        ...
        try {
            await api(`<?php echo e(url('/deals/stages')); ?>/${activeStageSection.dataset.stageId}`, 'DELETE');
    ...
    board?.addEventListener('click', async (event) => {
        ...
        if (stageColorOption) {
            ...
            try {
                const payload = await api(`<?php echo e(url('/deals/stages')); ?>/${section.dataset.stageId}`, 'PATCH', { color });
        ...
        if (stageAction) {
            ...
            if (action === 'move-left' || action === 'move-right') {
                try {
                    await api(`<?php echo e(url('/deals/stages')); ?>/${section.dataset.stageId}/move`, 'PATCH', {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\School\OJT\JK&CDealsContacts\jknc_project\resources\views\deals\index.blade.php ENDPATH**/ ?>