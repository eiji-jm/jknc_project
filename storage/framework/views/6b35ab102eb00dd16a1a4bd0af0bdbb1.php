    ...
    <div class="pointer-events-none absolute inset-y-0 right-0 flex w-full justify-end overflow-hidden">
        <div id="createDealPanel" class="pointer-events-auto flex h-full w-full max-w-[860px] translate-x-full flex-col border-l border-gray-200 bg-white shadow-2xl transition-transform duration-300 ease-out sm:max-w-[820px]">
            <div class="flex items-center justify-between border-b border-gray-100 px-6 py-5 sm:px-8">
                <div>
                    <h2 id="dealPanelTitle" class="text-2xl font-semibold text-gray-900"><?php echo e($panelTitle); ?></h2>
                    <p id="dealPanelSubtitle" class="mt-1 text-sm text-gray-500"><?php echo e($panelSubtitle); ?></p>
                </div>
                ...
                <div class="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-6 sm:px-8">
                    <?php if($errors->any()): ?>
                        <div id="dealFormErrorSummary" class="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700" role="alert" tabindex="-1">
                            ...
                            <p class="mt-1 text-xs text-red-600"><?php echo e(count($dealErrorMessages)); ?> <?php echo e(\Illuminate\Support\Str::plural('issue', count($dealErrorMessages))); ?> need attention.</p>
                            <ul class="mt-2 list-disc space-y-1 pl-5 text-xs text-red-700">
                                <?php $__currentLoopData = collect($dealErrorMessages)->take(6); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $errorMessage): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <div>
                                    ...
                                    <div id="service-area-options-grid" class="grid gap-2 sm:grid-cols-2">
                                        <?php $__currentLoopData = $serviceAreaOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <label class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700">
                                                <input type="checkbox" name="service_area_options[]" value="<?php echo e($option); ?>" <?php if(in_array($option, $selectedServiceAreas, true)): echo 'checked'; endif; ?> <?php echo e($option === 'Others' ? 'data-other-target=service-area-other-wrapper' : ''); ?> class="h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500">
                                ...
                                <table id="clientRequirementsTable" class="min-w-full border border-gray-200 text-sm">
                                    ...
                                    <tbody>
                                        <?php $__currentLoopData = $requirementRows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            ...
                                            ?>
                                            <?php if($key === 'others'): ?>
                                                <tr data-client-requirement-row="<?php echo e($key); ?>" data-client-others-row>
                                            <?php else: ?>
                                                <tr data-client-requirement-row="<?php echo e($key); ?>">
                                            <?php endif; ?>
                                                <td class="border border-gray-200 px-3 py-2 text-gray-700"><?php echo e($label); ?></td>
                                                <td class="border border-gray-200 px-3 py-2 text-center">
                                                    <input type="radio" name="requirements_status[<?php echo e($key); ?>]" value="provided" <?php if($current === 'provided'): echo 'checked'; endif; ?> class="h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500" data-client-requirement-status="<?php echo e($key); ?>:provided" <?php echo e($key === 'others' ? 'data-client-others-radio' : ''); ?>>
                                                </td>
                                                <td class="border border-gray-200 px-3 py-2 text-center">
                                                    <input type="radio" name="requirements_status[<?php echo e($key); ?>]" value="pending" <?php if($current === 'pending'): echo 'checked'; endif; ?> class="h-4 w-4 border-gray-300 text-amber-600 focus:ring-amber-500" data-client-requirement-status="<?php echo e($key); ?>:pending" <?php echo e($key === 'others' ? 'data-client-others-radio' : ''); ?>>
                                                </td>
                                            </tr>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <section class="rounded-2xl border border-gray-200 p-4">
                            ...
                            <div class="mt-3 grid gap-2 sm:grid-cols-2">
                                <?php $__currentLoopData = ['Full Payment Before Service', '50% Downpayment / 50% Completion', 'Milestone-Based Payment', 'Monthly Retainer', 'Others']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <label class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700">
                                        <input type="radio" name="payment_terms" value="<?php echo e($option); ?>" <?php if(old('payment_terms', $draft['payment_terms'] ?? '') === $option): echo 'checked'; endif; ?> <?php echo e($option === 'Others' ? 'data-other-target=deal_payment_terms_other_wrap' : ''); ?> class="h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500">
                                        <span><?php echo e($option); ?></span>
                                    </label>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        ...
                        <section class="rounded-2xl border border-gray-200 p-4">
                            ...
                            <div class="mt-3 grid gap-2 sm:grid-cols-2">
                                <?php $__currentLoopData = ['Prepare Proposal', 'Prepare Engagement Letter', 'Schedule Client Consultation', 'Request Additional Documents', 'Decline Engagement']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <label class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700">
                                        <input type="radio" name="proposal_decision" value="<?php echo e($option); ?>" <?php if(old('proposal_decision', $draft['proposal_decision'] ?? '') === $option): echo 'checked'; endif; ?> <?php echo e($option === 'Decline Engagement' ? 'data-other-target=deal_decline_reason_wrap' : ''); ?> class="h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500">
                                        <span><?php echo e($option); ?></span>
                                    </label>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const invalidFieldMessages = <?php echo json_encode($dealErrorMap, 15, 512) ?>;
    const invalidFieldKeys = <?php echo json_encode($dealErrorKeys, 15, 512) ?>;
    const shouldAutoOpenModal = <?php echo json_encode($openDealModal || $errors->any(), 15, 512) ?>;
